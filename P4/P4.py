...
class mrGraphAlg(MRJob):
        ...
        def mapper(self, node, neighbors):
                ...
                for (n, d) in neighbors:
                        # update distance for next node. dist to current node + dist to neighbor
                        yield n, ("distance_type",d + dist)

        def reducer(self, node, values):
                updated_neighbors = []
                min_dist = 999
                # if counter is never hit later, it won't exist, so initialize it for every run
                ...
                for(itemtype,itemvalue) in values:
                        ...
                        if (itemtype == "graph_type"):
                                ...
                                # we received updated dist, pick b/t the min dist and the received dist
                ...
                if(min_dist < curr_dist):
                        ...
                        # we will converge if no distances are updated
